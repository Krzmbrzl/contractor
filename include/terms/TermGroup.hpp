...
public:
	...
	void setTerms(const std::vector< term_t > &terms) { m_terms = terms; }
	void setTerms(std::vector< term_t > &&terms) { m_terms = std::move(terms); }
	void addTerm(const term_t &term) { m_terms.push_back(term); }
	void addTerm(term_t &&term) { m_terms.push_back(std::move(term)); }
